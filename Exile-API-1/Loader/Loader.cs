...
public class Loader
{
	...
	{
		string text = Path.Join(AppDomain.CurrentDomain.BaseDirectory, "ExileCore.dll");
		CoreAssemblyLoadHelper coreAssemblyLoadHelper = new CoreAssemblyLoadHelper(text);
		AssemblyLoadContext.Default.Resolving += coreAssemblyLoadHelper.ResolvingCallback;
		AssemblyLoadContext.Default.ResolvingUnmanagedDll += coreAssemblyLoadHelper.ResolvingUnmanagedDllCallback;
	...
	{
		GetTypeFromCoreDll("ExileCore.CommandExecutor").GetMethod("Execute").Invoke(null, new object[1] { command });
	}

	private void LoadLogger()
	{
		PropertyInfo propertyInfo = GetTypeFromCoreDll("ExileCore.Logger").GetProperty("Log") ?? throw new InvalidOperationException("Not found Log property in Logger class.");
	...
	{
		MeasurePerformance("Create new offsets", delegate
		{
			ExecuteCommand("loader_offsets");
		});
	}
	...
	{
		MethodInfo? method = _performanceTimerType.GetMethod("Dispose");
		object obj = CreatePerformanceTimer(actionName);
		action();
	...
	private void LogHudLoadedMessage()
	{
		...
		GetTypeFromCoreDll("ExileCore.DebugWindow").GetMethod("LogMsg", new Type[3]
		{
			typeof(string),
			typeof(float),
			typeof(Color)
		...
		{
			$"HUD loaded in {_stopwatch.Elapsed.TotalMilliseconds} ms.",
			7,
			Color.GreenYellow
		});
	}
	...
	{
		GetTypeFromCoreDll("ExileCore.DebugWindow").GetMethod("LogError").Invoke(null, new object[2]
		{
			e.ToString(),
			2
		});
	...
	{
		_logger.Information($"{"==============="} Start hud at {DateTime.Now} {"==============="}");
	}

	private void LogCloseMessage()
	{
		_logger.Information($"{"==============="} Close hud at {DateTime.Now} {"==============="}");
	}
}
